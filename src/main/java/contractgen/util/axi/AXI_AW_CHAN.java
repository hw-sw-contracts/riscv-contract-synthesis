...
public class AXI_AW_CHAN {
    ...
    private final int atop;
    private final int user;

    public AXI_AW_CHAN(int id, long addr, int len, int size, int burst, int lock, int cache, int prot, int qos, int region, int atop, int user) {
        this.id = id;
        this.addr = addr;
        this.len = len;
        this.size = size;
    ...
    public static AXI_AW_CHAN fromString(String binary) {
        binary = StringUtils.expandToLength(binary, AXI_AW_WIDTH, '0');
        return new AXI_AW_CHAN(
                ...
                Integer.parseInt(binary.substring(AXI_ID_WIDTH + AXI_ADDR_WIDTH + AXI_LEN_WIDTH + AXI_SIZE_WIDTH + AXI_BURST_WIDTH + AXI_LOCK_WIDTH + AXI_CACHE_WIDTH + AXI_PROT_WIDTH + AXI_QOS_WIDTH + AXI_REGION_WIDTH, AXI_ID_WIDTH + AXI_ADDR_WIDTH + AXI_LEN_WIDTH + AXI_SIZE_WIDTH + AXI_BURST_WIDTH + AXI_LOCK_WIDTH + AXI_CACHE_WIDTH + AXI_PROT_WIDTH + AXI_QOS_WIDTH + AXI_REGION_WIDTH + AXI_ATOP_WIDTH), 2),
                Integer.parseInt(binary.substring(AXI_ID_WIDTH + AXI_ADDR_WIDTH + AXI_LEN_WIDTH + AXI_SIZE_WIDTH + AXI_BURST_WIDTH + AXI_LOCK_WIDTH + AXI_CACHE_WIDTH + AXI_PROT_WIDTH + AXI_QOS_WIDTH + AXI_REGION_WIDTH + AXI_ATOP_WIDTH, AXI_ID_WIDTH + AXI_ADDR_WIDTH + AXI_LEN_WIDTH + AXI_SIZE_WIDTH + AXI_BURST_WIDTH + AXI_LOCK_WIDTH + AXI_CACHE_WIDTH + AXI_PROT_WIDTH + AXI_QOS_WIDTH + AXI_REGION_WIDTH + AXI_ATOP_WIDTH + AXI_USER_WIDTH), 2)
        );
    }
